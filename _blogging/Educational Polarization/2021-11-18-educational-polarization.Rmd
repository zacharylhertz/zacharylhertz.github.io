...
```{r, include=FALSE}
dat <- dat %>%
  subset(year == 2008 | year == 2012 | year == 2016 | year == 2020) %>%
  mutate(college = 
           case_when(educ < 3 ~ "No college",
                     educ > 3 ~ "College"),
         race2 = 
           case_when(race == 1 ~ "White Americans",
                     race == 2 ~ "Black Americans",
                     race == 3 ~ "Hispanic Americans",
                     race == 4 ~ "Asian Americans"),
         newpid1 =
           case_when(pid3_leaner==1 ~ 1,
                     pid3_leaner!=1 ~ 0),
         newpid2 =
           case_when(pid3_leaner==2 ~ 1,
                     pid3_leaner!=2 ~ 0),
         newpid3 = 
           case_when(pid3_leaner==3 ~ 1,
                     pid3_leaner!=3 ~ 0),
         intent.tp = 
...
```{r}
ggplot(intent, 
       aes(x=year, y=intent.tp, group = college, color = college)) +
  geom_line() + geom_point() + 
  geom_errorbar(aes(ymin=intent.tp-1.41*se, 
                  ymax=intent.tp+1.41*se), 
              width=.2,
              position=position_dodge(0.05)) + 
  ylab("Democratic Share of Two-Party Vote Intention") + 
  xlab("Year") +
  theme_minimal() + 
  ...
  scale_color_manual(values=c("#3D315B", "#80D39B")) +
  facet_wrap(~race2, ncol=4)
ggsave("intent-race.png", width = 10, height = 5, units = c("in"), dpi=700)

ggplot(voted, 
       aes(x=year, y=voted.tp, group = college, color = college)) +
  geom_line() + geom_point() + 
  geom_errorbar(aes(ymin=voted.tp-1.41*se, 
                  ymax=voted.tp+1.41*se), 
              width=.2,
              position=position_dodge(0.05)) + 
  ylab("Democratic Share of Two-Party Vote") + 
  xlab("Year") +
  theme_minimal() + 
  theme(legend.title = element_blank(),
        legend.position = "bottom",
...
ggsave("voted-race.png", width = 6, height = 6, units = c("in"), dpi=700)
...
ggplot(pid, 
  ...
  geom_errorbar(aes(ymin=voted.tp-1.41*se, 
              ...
              width=.2,
              position=position_dodge(0.05)) + 
  ylab("Democratic Share of Two-Party Vote Intention") + theme_minimal() + facet_wrap(~pid3_leaner)
```

```{r}
ggplot(pid1, 
       aes(x=year, y=newpid1, group = college, color = college)) +
  geom_line() + geom_point() + 
  geom_errorbar(aes(ymin=newpid1-1.41*se, 
                  ymax=newpid1+1.41*se), 
              width=.2,
              position=position_dodge(0.05)) + 
  ylab("Share Identifying as Democrats (leaners included)") +
  xlab("Year") +
  theme_minimal() +
  theme(legend.title = element_blank(),
        legend.position = "bottom",
...
ggsave("pid-dem-race.png", width = 6, height = 6, units = c("in"), dpi=700)
```

```{r}
ggplot(pid2, 
       aes(x=year, y=newpid2, group = college, color = college)) +
  geom_line() + geom_point() + 
  geom_errorbar(aes(ymin=newpid2-1.41*se, 
                  ymax=newpid2+1.41*se), 
              width=.2,
              position=position_dodge(0.05)) + 
  ylab("Share Identifying as Republicans (leaners included)") +
  xlab("Year") +
  theme_minimal() +
  theme(legend.title = element_blank(),
        legend.position = "bottom",
...
ggsave("pid-rep-race.png", width = 6, height = 6, units = c("in"), dpi=700)
```

```{r}
ggplot(pid3, 
       aes(x=year, y=newpid3, group = college, color = college)) +
  geom_line() + geom_point() + 
  geom_errorbar(aes(ymin=newpid3-1.41*se, 
                  ymax=newpid3+1.41*se), 
              width=.2,
              position=position_dodge(0.05)) + 
  ylab("Share Identifying as Independents (leaners included)") +
  xlab("Year") +
  theme_minimal() +
  theme(legend.title = element_blank(),
        legend.position = "bottom",
