...
# Bruce Lou vs Trump Performance Comparison
...
# Relationship between Asian population and Republican overperformance
...
p10 <- ggplot(final_data, aes(x = pelosi.twv_22, y = resid.asian)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  theme_minimal()

p11 <- ggplot(final_data, aes(x = asian_pct, y = bruce_netpct)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  labs(
    x = "Asian American Population (%)",
    ...
    title = "Lou performance relative to citywide results \nand Asian American population"
  ) +
  theme(
    plot.title = element_text(hjust = 0.5)
  ) +
...
p13 <- ggplot(final_data, aes(x = asian_pct, y = pelosi.twv_shift)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  labs(
    x = "Asian American Population (%)",
    ...
    caption = "Plot: Zachary L. Hertz\nData: Statewide Database, IPUMS",
    title = "CA-11 precinct-level 2022-2024 shift relative to \ncitywide results and Asian American population"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5)
...
p16 <- ggplot(final_data, aes(x = pelosi.twv_22, y = resid.asian)) +
  geom_point() +
  geom_smooth(method=lm, se = FALSE) +
  labs(
    x = "Pelosi 2022 Vote Share",
    y = "Residual From Regression of Lou Vote Share on Asian Population",
    caption = "Plot: Zachary L. Hertz\nData: Statewide Database, IPUMS",
    title = "Prediction error from a model using only Asian \npopulation is correlated with 2022 results"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5)
...
p17 <- ggplot(final_data, aes(x = asian_pct, y = resid.hist)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  labs(
    x = "Population Percent Asian American",
    y = "Residual From Regression of Lou Vote Share on 2022 CA-11 results",
    caption = "Plot: Zachary L. Hertz\nData: Statewide Database, IPUMS",
    title = "Prediction error from a model using only 2022 results \nis not strongly correlated with Asian population"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5)
...
p18 <- ggplot(final_data, aes(x = asian_pct, y = resid.trump)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  labs(
    x = "Population Percent Asian American",
    y = "Residual From Regression of Lou Vote Share on Trump Vote Share",
    caption = "Plot: Zachary L. Hertz\nData: Statewide Database, IPUMS",
    title = "Prediction error from a model using only 2024 \npresidential results is not correlated with Asian population"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5)
...
p19 <- ggplot(final_data, aes(x = blau_index, y = resid.trump)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  labs(
...
# Save plots -----------------------------------------------
ggsave("images/results_2024_map.png", p2)
ggsave("images/aa_pop_map.png", p3)
ggsave("images/scatter_lou_aa.png", p11)
ggsave("images/normalized_22_24_shift.png", p12)
ggsave("images/scatter_22_24_shift.png", p13)
ggsave("images/headline_22_24_shift.png", p14)
ggsave("images/residuals_22vote.png", p16)
ggsave("images/residuals_asianpop.png", p17)
ggsave("images/residuals_asianpoppres.png", p18)
ggsave("images/blau_map.png", p5)
ggsave("images/residuals_blau.png", p19)



# Objects available for further analysis:
#   - final_data: Complete dataset with all variables
#   - p1-p8: Various plots and visualizations
#   - pal, pal2: Color palettes for interactive maps